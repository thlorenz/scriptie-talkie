...
function evalNextSnippet(remainingSnippets, ctx, onAllSnippetsEvaluated) {
  ...
  var result  = contextifyRunner.runInCtx(snippet.code, ctx);
  ...
  var crntCtx          =  clone(ctx)
    ...
    // if a script attaches a global _cb function, hold your horses and proceed as follows
    // 1. execute next script
    // 2. wait for _cb to be invoked, evaluate context and arguments
    // 3. delete _cb from the global object
    // 4. execute next script
    // first we need async execution support (recursion?)

    function onSnippetEvaluated() {
      snippet.evaluated = {
          result  :  result
        , ctx     :  prevCtx 
