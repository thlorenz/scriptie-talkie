...
function evalSnippet(ctx, snippet, isStrict, allAddedKeyValues) {
...
module.exports = function evalSnippets(snippets, fullSrcPath) {
  ...
  if (!browser) {
    ...
    fndecs.forEach(function(snippet) { evalSnippet(ctx, snippet, isStrict, allAddedKeyValues); });
  }

  nonEmptySnippets.forEach(function(snippet) { evalSnippet(ctx, snippet, isStrict, allAddedKeyValues); });
  return ctx;
};
